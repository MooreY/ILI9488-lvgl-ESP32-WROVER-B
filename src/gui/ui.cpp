...
ui_object_t* UI::add_object(lv_obj_t* object, ui_object_id id, ui_object_type object_type, ui_object_handler_cb_ptr callback) {
...
ui_list_item_t* UI::object_id_in_list(ui_list_item_t* list, ui_object_t* ui_object) {
	for (int i = 0; i < 4; i++) {
...
ui_object_t* UI::create_page(ui_object_id parent_id, ui_object_id id, uint32_t width, uint32_t height) {
...
ui_object_t* UI::create_tabview(ui_object_id parent_id, ui_object_id id, ui_tab_item_t* tabs, uint32_t num_tabs, uint32_t width, uint32_t height) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
...
ui_object_t* UI::create_container(ui_object_id parent_id, ui_object_id id, uint32_t width, uint32_t height) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
...
ui_object_t* UI::add_textarea(ui_object_id parent_id, ui_object_id id, bool use_keyboard, bool oneline, bool pw_mode, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
	lv_obj_t* lv_obj = lv_ta_create(lv_parent, NULL);
	if (lv_style != nullptr)
		lv_label_set_style(lv_obj, LV_LABEL_STYLE_MAIN, lv_style);
	if (align < 999) {
	...
	ui_object_t* ui_obj = add_object(lv_obj, id, ui_object_type::TEXTAREA, callback);
	return ui_obj;
}

ui_object_t* UI::add_label(ui_object_id parent_id, ui_object_id id, const char* text, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
...
ui_object_t* UI::add_switch(ui_object_id parent_id, ui_object_id id, bool state, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
...
ui_object_t* UI::add_spinbox(ui_object_id parent_id, ui_object_id id, ui_spinbox_specs* object_specs, ui_object_handler_cb_ptr callback, lv_align_t align,
	ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
	...
	lv_obj_set_event_cb(lv_obj, UI::event_handler);
	ui_object_t* ui_obj = add_object(lv_obj, id, ui_object_type::SPINBOX, callback);
	ui_obj->object_data = object_specs;
	return 	ui_obj;
}

ui_object_t* UI::add_button(ui_object_id parent_id, ui_object_id id, ui_object_id label_id, const char* label, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
...
ui_object_t* UI::add_checkbox(ui_object_id parent_id, ui_object_id id, const char* label, bool state, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
...
ui_object_t* UI::add_led(ui_object_id parent_id, ui_object_id id, bool state, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset, const lv_style_t* lv_style) {
	lv_obj_t* lv_parent = find_parent_object(parent_id);
	...
	if (lv_style != nullptr)
		lv_label_set_style(lv_obj, LV_LED_STYLE_MAIN, lv_style);
...
void UI::keyboard_event_cb(lv_obj_t * kb, lv_event_t event)
{
	...
	if (event == LV_EVENT_APPLY || event == LV_EVENT_CANCEL) {
		...
		full_keyboard = NULL;
#endif
//		lv_obj_set_size(ta, lv_obj_get_width_fit(parent), lv_obj_get_height_fit(parent));
	}
}
...
void UI::set_value(ui_object_id id, int num_args, ...) {
	...
	case ui_object_type::LABEL: {
		...
		lv_label_set_text(lv_obj, (const char*)value);
		break;
	}
	case ui_object_type::BUTTON: {
		const char* value = va_arg(arguments, const char*);
	...
	case ui_object_type::SLIDER: {
		...
		lv_slider_set_value(lv_obj, value, true);
		break;
	}
	case ui_object_type::BAR: {
		int value = va_arg(arguments, int);
		lv_bar_set_value(lv_obj, value, true);
		break;
	}
	case ui_object_type::SWITCH: {
...
void UI::set_style(ui_object_id id, ui_styles_t* ui_styles, uint16_t num_items) {
	ui_object_t* ui_obj = find_object_by_id(id);
	switch (ui_obj->type) {
	case ui_object_type::TABVIEW:
		for (int i = 0; i < num_items; i++) {
			lv_tabview_set_style(ui_obj->lv_obj, ui_styles[i].style_type, ui_styles[i].style);
		}
		break;
	}
...
ui_object_t* UI::add_dropdown(ui_object_id parent_id, ui_object_id id, ui_dropdown_specs* specs, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset) {
	...
	lv_obj_set_event_cb(lv_obj, UI::event_handler);
	ui_object_t* ui_obj = add_object(lv_obj, id, ui_object_type::DROPDOWN_LIST,callback);
...
ui_object_t* UI::add_roller(ui_object_id parent_id, ui_object_id id, ui_roller_specs* specs, ui_object_handler_cb_ptr callback, lv_align_t align, ui_object_id base_id, uint32_t x_offset, uint32_t y_offset) {
	...
	lv_obj_set_event_cb(lv_obj, UI::event_handler);
	ui_object_t* ui_obj = add_object(lv_obj, id, ui_object_type::ROLLER,callback);
	ui_obj->object_data = specs;
	return 	ui_obj;
}
